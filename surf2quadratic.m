...
%
% [3] Chung, M.K., 2013 Computational Neuroanatomy: The Methods, World
%     Scientific Publishing, pages 100-109
% 
% (C) 2019 Moo K. Chung mkchung@wisc.edu
% University of Wisconsin-Madison
%
% Update histroy: Created 2001; Updated: 2007 Nov. 16; 
...
for i=1:n_vertex
   ...
   % Update 2026 April 19
   % Solve least squares with fallback stabilization

   warnID = 'MATLAB:nearlySingularMatrix';  % removes ill-conditioning warning                        
   ws = warning('off',warnID); 

   if rcond(X'*X) > 1e-10
       beta(i,:) = (X\z)';
   else
       beta(i,:) = (pinv(X)*z)';   % stabilize ill-conditioned case casued by badly constructed mesh
   end

   warning(ws.state,warnID);
